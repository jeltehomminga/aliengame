<template>
  <div id="app" class="contain">
    <GamestateStart v-if="uiState === 'start'">
      ...
      >
        <label>
          <input v-model="characterinput" type="radio" :value="option" />
          {{ option }}
        </label>
      </p>
      <button @click="pickCharacter">Pick your character</button>
    </GamestateStart>
    <GamestateFinal v-else-if="uiState === 'won' || uiState === 'lost'">
      You've {{ uiState === 'won' ? 'won' : 'lost' }}
    </GamestateFinal>
    <section v-else>
...
import GamestateFinal from '@/components/GamestateFinal.vue';
